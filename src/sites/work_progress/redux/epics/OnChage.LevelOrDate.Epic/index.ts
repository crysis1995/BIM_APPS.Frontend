...
export const OnChangeLevelOrDateEpic: Epic<RootActions, RootActions, RootState> = (
	action$,
	state$,
) =>
	combineLatest([
		action$.pipe(
			filter(
				(
					...
					WorkProgress.Monolithic.General.Redux.IActions['SetActiveTab']
		...
		action$.pipe(
			filter(
				(
					...
					WorkProgress.Monolithic.General.Redux.IActions['SetRotationDay']
		...
		action$.pipe(
			filter(
				(
					...
					WorkProgress.Monolithic.Upgrading.Redux.IActions['FetchEnd']
		...
		filter(
			([actions, state]) =>
				!state.ForgeViewer.model_elements_loading &&
				!!state.WorkProgress.Monolithic.General.active_level,
		),
