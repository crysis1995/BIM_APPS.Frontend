	...
	action$.pipe(
		filter(
			(
				...
				WorkProgress.Monolithic.General.Redux.IActions['TrySetRotationDay']
		...
		switchMap(([action, state]) => {
			if (
				...
				if (
					...
					date =
						state.WorkProgress.Monolithic.General.calendar_all[calendarDateID].date_id
							.data;
				}
