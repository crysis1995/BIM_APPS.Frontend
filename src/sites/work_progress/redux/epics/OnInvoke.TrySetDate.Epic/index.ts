			...
			if (
				...
				if (action.payload in state.WorkProgress.Monolithic.General.calendar_by_dates) {
					const calendarDateID =
						state.WorkProgress.Monolithic.General.calendar_by_dates[action.payload];
					rotationDay =
						state.WorkProgress.Monolithic.General.calendar_all[calendarDateID]
							.rotation_day;
				} else {
					isValid = false;
					let current = GetFormattedDate(
						dayjs(action.payload).subtract(1, 'day'),
						FormatType.Day,
					);
					...
					if (current in state.WorkProgress.Monolithic.General.calendar_by_dates) {
						const calendarDateID =
							state.WorkProgress.Monolithic.General.calendar_by_dates[current];
						rotationDay =
							state.WorkProgress.Monolithic.General.calendar_all[calendarDateID]
								.rotation_day;
					} else rotationDay = 0;
