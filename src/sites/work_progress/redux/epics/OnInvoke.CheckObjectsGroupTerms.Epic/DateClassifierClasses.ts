...
export class FinishUpdateIsNeeded {
	...
	constructor(objectsCount: number, term: GetAllAcceptanceTermsType.AcceptanceTerm) {
	...
	qualifiedStatus(item: GetObjectsByLevelType.AcceptanceObject) {
		if (this.updating) {
			const finishedStatus = item.statuses.find((x) => x.status === GetObjectsByLevelType.StatusEnum.Finished);
			if (finishedStatus) {
