...
export const prepareDataForJobs = (job_id, objects, precision = 2) => {
	...
	for (let object_id in objects) {
		...
		//
		const isContainsJobId = objects[object_id].object_finish_type.jobs.reduce((previous, job) => previous || job.id === job_id, false);
...
export const fetchSummaryValuesByJob = async (job_id, current_level, precision) => {
	return new Promise((resolve, reject) => {
		...
		var elements;
		Promise.all([fetchALLAreaJobPerLevel(job_id, current_level), fetchSummaryAreaJobPerLevel(job_id, current_level)]).then((resp) => {
			if (resp[0].data) summary_all_value = Math.floor((resp[0].data.acceptanceObjectsConnection.aggregate.sum.area || 0) * 10 ** precision) / 10 ** precision;
			if (resp[1].data) {
				summary_current_value = Math.floor((resp[1].data.acceptanceReferenceJobsConnection.aggregate.sum.value_calculated || 0) * 10 ** precision) / 10 ** precision;
			...
			percentage_value = Math.floor((summary_current_value / summary_all_value) * 100 * 10 ** precision) / 10 ** precision;
			resolve({
				summary_all_value,
