...
export const OnStartGeneralConstructionComponentEpic: Epic<RootActions, RootActions, RootState> = (
	action$,
	state$,
) =>
	combineLatest([
		action$.pipe(
			ofType(WorkProgress.GeneralConstruction.General.Redux.Types.COMPONENT_STARTED),
		),
		...
		mergeMap(([_, state]) => {
			...
			return merge(FetchObjects(GRAPHQL, project)).pipe(
				takeUntil(
					action$.pipe(
						ofType(
							WorkProgress.GeneralConstruction.General.Redux.Types.COMPONENT_ENDED,
						),
					),
				),
			);
