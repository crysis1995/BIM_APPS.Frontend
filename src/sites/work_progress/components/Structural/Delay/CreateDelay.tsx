...
const mapStateToProps = (state: {
	Odbiory: {
		OdbioryComponent: { MONOLITHIC: { active_crane: string; active_level: string; rotation_day: number } };
		Delays: { MONOLITHIC: { byCrane: any; delay_causes_loading: boolean; delay_causes: DelayCause[] } };
	};
}) => ({
...
function CreateDelay({ initCreateNewDelay, delay_causes_loading, delay_causes, active_crane, active_level }: Props) {
	...
	if (!active_crane || !active_level) return null;
	if (delay_causes_loading) return <Loader />;

	function ChildCheck({ object, margin }: { object: DelayCause; margin: number }) {
...
export default connect(mapStateToProps, mapDispatchToProps)(CreateDelay);
