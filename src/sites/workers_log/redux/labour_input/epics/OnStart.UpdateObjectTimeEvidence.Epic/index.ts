...
export const OnStartUpdateObjectTimeEvidenceEpic: Epic<RootActions, RootActions, RootState> = (action$, state$) =>
	action$.pipe(
		filter(
			(
				data,
			...
			> => data.type === WorkersLog.LabourInput.Redux.TimeEvidence.Types.UPDATE_OBJECT_TIME_EVIDENCE_START,
		),
		withLatestFrom(state$),
		switchMap(([action, state]) => {
			console.log(action.payload.workedTime);
			const UpdateObjectWorkTimeEvidenceAPI = new GraphQLAPIService(state.CMSLogin.credentials?.access_token)
