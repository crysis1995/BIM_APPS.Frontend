...
const mapDispatchToProps = {
	...
	UngroupObjects: LabourInputObjectsActions.UngroupObjects,
};
...
function GroupHeader(props: Props) {
	...
	useEffect(() => {
		...
		if (ref.current) {
			const objectsIncludedInSelection = objects.filter((x) => props.Selection.includes(x));
	...
	function handleSelect() {
		props.SelectObject(props.groupedObject.objects);
	}
	...
	return (
		<tr className={classNames({ collapse: true, show: props.show })}>
			<td colSpan={3} className={'pl-3'}>
				...
				/>
				<span className={'ml-2'}>{props.groupedObject.name}</span>
				...
				<Button
					...
					size={'sm'}
					variant={'light'}
					onClick={() => props.UngroupObjects(props.groupedObject.id)}>
					<svg
						version="1.1"
						id="Layer_1"
						className={'mr-2'}
