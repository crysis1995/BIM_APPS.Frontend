			...
			<Table className="mt-3" hover>
				...
				<tbody>
					{Object.keys(props.Jobs.jobs)
						...
						.map((job_key) => (
							<tr key={v4()} className={classNames({ 'table-active': job_key === props.active_job_id })}>
								<OverlayTrigger
									placement={'top'}
									overlay={
										<Tooltip id={`tooltip-top`}>
											{[
												...new Set(
													props.Jobs.jobs[job_key].upgrading.object_ids.map((object_id) => props.Objects.objects[object_id].object_finish_type.name)
												),
											].map((e) => (
								...
								<OverlayTrigger
									...
									overlay={
										<Tooltip id={`tooltip-top`}>
											{
												<>
													<span>{props.Jobs.jobs[job_key].upgrading.particular_values.join('+')}</span>
												</>
											}
										</Tooltip>
									}>
...
const mapStateToProps = ({ Odbiory }) => ({
	...Odbiory,
	active_job_id: Odbiory.Results.active_job_id,
});
