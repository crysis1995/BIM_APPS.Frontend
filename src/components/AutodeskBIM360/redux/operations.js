...
export const fetchAllProjects = () => async (dispatch, getState) => {
	...
	const projects = await api.bim360.getProjects(getState().AutodeskBIM360.accountId);
...
export const getProjectTopFolders = () => async (dispatch, getState) => {
	if (getState().AutodeskBIM360.currentProject) {
		const api = new AutodeskApi(getState().AutodeskLogin.login_3_legged.access_token);
		const folders = await api.bim360.getProjectTopFolders(getState().AutodeskBIM360.accountId, getState().AutodeskBIM360.currentProject);
